...
pub trait ItemPersistence: Send + Sync {
    ...
    async fn get_items_by_column_with_limit_offset(&self, column_id: Uuid, limit: usize, offset: usize) -> Result<Vec<Item>>;
...
impl ItemUseCase {
    ...
    pub async fn get_item_history(&self, item_id: Uuid, action_user: Uuid) -> Result<Vec<ItemHistory>> {
    ...
    pub async fn get_items(&self, column_id: Uuid, limit: usize, offset: usize, action_user: Uuid) -> Result<Vec<Item>> {
        let column = self
            .column_persistence
        ...
        self.item_persistence.get_items_by_column_with_limit_offset(column_id, limit, offset).await
    }
    ...
    pub async fn update_item_details(
        ...
        let mut item = self.item_persistence.get_item(item_id).await?
            .ok_or(AppError::ResourceNotFound("Item", item_id))?;
    ...
    pub async fn move_item(&self, item_id: Uuid, new_column: Uuid, action_user: Uuid) -> Result<()> {
        let mut item = self.item_persistence.get_item(item_id).await?
            .ok_or(AppError::ResourceNotFound("Item", item_id))?;
        ...
        self.item_persistence.update_item(&item, Some(&history)).await?;
