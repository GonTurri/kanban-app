...
impl Item {
    pub fn new(
       ...
       let item = Self {
        ...
        let first_history = ItemHistory::new(item.id, None, item.column_id);

        (item, first_history)
    }

    pub fn move_to_column(&mut self, column_id: Uuid, is_done: bool) -> DomainResult<ItemHistory> {
        ...
        Ok(ItemHistory::new(self.id, Some(prev_column), self.column_id))
    }
